...
public class CountryController {
	...
	@PostConstruct
	private void check() {
		log.info("Check postconstruct called service Up : "+(countryService!=null?"Yes":"No"));
	}
	...
	public ModelAndView countryModify(@ModelAttribute("countryForm") @Valid Country country, ModelMap model) {
		log.trace("Appel countryModify : {} ", country);
		countryService.create(country);
		return new ModelAndView("countryModify","country",country);
	}

	@GetMapping("/countryDelete")
	public String deleteCountry(@RequestParam(name="id") Long id, ModelMap model) {
		log.trace("Appel countryDelete : id={} ", id);
